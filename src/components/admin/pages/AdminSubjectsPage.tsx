      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-xl">
          ...
          <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="border rounded-md px-3 py-2 md:col-span-2">
              <p className="text-xs text-gray-500 mb-2">Grades</p>
              <div className="grid grid-cols-3 gap-2">
                {FORM_OPTIONS.map((formOption) => (
                  <label key={formOption} className="inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={form.grades.includes(formOption)}
                      onChange={(e) => {
                        const updated = e.target.checked
                          ? [...form.grades, formOption]
                          : form.grades.filter((g) => g !== formOption);
