...
type TabId = 'account' | 'personal' | 'professional' | 'teaching';
...
const defaultTeacherForm: TeacherFormData = {
  ...
  gender: '', dateOfBirth: '', nationalId: '', maritalStatus: '', homeAddress: '',
  spouseName: '', spousePhone: '', numberOfDependants: '',
  emergencyContactName: '', emergencyContactPhone: '', emergencyContactRelationship: '',
  staffNumber: '', teachingCouncilRegNumber: '', employmentType: '', position: '',
  employmentStartDate: '', qualifications: '', teachingCertificate: '',
...
function fromUser(user: User): TeacherFormData {
  ...
  return {
    ...
    spouseName: tp?.spouseName || '',
    spousePhone: tp?.spousePhone || '',
    numberOfDependants: String(tp?.numberOfDependants ?? ''),
    ...
    teachingCouncilRegNumber: tp?.teachingCouncilRegNumber || '',
    employmentType: tp?.employmentType || '',
    position: tp?.position || '',
    employmentStartDate: tp?.employmentStartDate ? String(tp.employmentStartDate).substring(0, 10) : '',
    ...
    subjectAssignments: (tp?.subjectAssignments || []).map((sa: any) => ({
      ...
      classes: (sa.classes || []).map((c: any) => (typeof c === 'object' ? c._id : c)),
    })),
  };
}
...
const TeacherFormModal: React.FC<Props> = ({ open, mode, user, saving, onClose, onSubmit }) => {
  ...
  const removeAssignment = (i: number) =>
    setForm((p) => ({ ...p, subjectAssignments: p.subjectAssignments.filter((_, idx) => idx !== i) }));
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === 'create'
              ? 'Add a new teacher with full profile and teaching assignment details.'
              : 'Update teacher account, profile, and teaching assignments.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {tab === 'personal' && (
            <div className="space-y-5">
              <div>
                ...
                <div className={sectionCls}>
                  ...
                  <div>
                    ...
                    <input className={inputCls} placeholder="e.g. 63-123456-A-00" value={form.nationalId}
                      onChange={(e) => set('nationalId', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Marital Status</label>
                    <select className={inputCls} value={form.maritalStatus}
              ...
              <div>
                <h4 className={subheadCls}>Spouse / Family</h4>
                <div className={sectionCls}>
                  <div>
                    <label className={labelCls}>Spouse Name</label>
                    <input className={inputCls} placeholder="Spouse full name (if married)"
                      value={form.spouseName} onChange={(e) => set('spouseName', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Spouse Phone</label>
                    <input className={inputCls} placeholder="e.g. +263 77 123 4567"
                      value={form.spousePhone} onChange={(e) => set('spousePhone', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Number of Dependants</label>
                    <input type="number" min={0} className={inputCls} placeholder="0"
                      value={form.numberOfDependants} onChange={(e) => set('numberOfDependants', e.target.value)} />
                  </div>
                </div>
              </div>

              <div>
                <h4 className={subheadCls}>Emergency Contact</h4>
                <div className={sectionCls}>
                  <div>
                    <label className={labelCls}>Contact Name</label>
                    <input className={inputCls} placeholder="Full name" value={form.emergencyContactName}
                      onChange={(e) => set('emergencyContactName', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Contact Phone</label>
                    <input className={inputCls} placeholder="Phone number" value={form.emergencyContactPhone}
                      onChange={(e) => set('emergencyContactPhone', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Relationship</label>
                    <input className={inputCls} placeholder="e.g. Spouse, Parent, Sibling"
          ...
          {tab === 'professional' && (
            <div className="space-y-5">
              <div>
                <h4 className={subheadCls}>Employment Details</h4>
                <div className={sectionCls}>
                  <div>
                    <label className={labelCls}>Staff / Employee Number</label>
                    <input className={inputCls} placeholder="e.g. EMP-0042"
                      value={form.staffNumber} onChange={(e) => set('staffNumber', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Teaching Council Reg Number</label>
                    <input className={inputCls} placeholder="ZTC registration number"
                      value={form.teachingCouncilRegNumber}
                      onChange={(e) => set('teachingCouncilRegNumber', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Employment Type</label>
                    <select className={inputCls} value={form.employmentType}
                  ...
                  <div>
                    ...
                    <input className={inputCls} placeholder="e.g. Head of Department, Class Teacher"
                      value={form.position} onChange={(e) => set('position', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Employment Start Date</label>
                    <input type="date" className={inputCls} value={form.employmentStartDate}
                      onChange={(e) => set('employmentStartDate', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelCls}>Department</label>
                    <input className={inputCls} placeholder="e.g. Sciences, Humanities, Commercial"
              ...
              <div>
                <h4 className={subheadCls}>Qualifications</h4>
                <div className={sectionCls}>
                  <div>
                    <label className={labelCls}>Highest Qualification</label>
                    <input className={inputCls} placeholder="e.g. B.Ed, M.Ed, HND, Dip Ed"
                  ...
                  <div>
                    ...
                    <input className={inputCls} placeholder="e.g. PGCE, Dip Ed, B.Ed"
                      value={form.teachingCertificate}
                      onChange={(e) => set('teachingCertificate', e.target.value)} />
                  </div>
                  <div>
          ...
          {tab === 'teaching' && (
            <div className="space-y-6">
              <div>
                <h4 className={subheadCls}>Class Teacher Assignment</h4>
                <p className="text-xs text-gray-500 mb-3">The class this teacher is the form/class teacher for.</p>
                <select className={inputCls} value={form.classTeacherOf}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h4 className={subheadCls} style={{ marginBottom: 0 }}>Subject Assignments</h4>
                  <button
                    type="button"
                ...
                <p className="text-xs text-gray-500 mb-3">For each subject, choose which classes the teacher delivers it to.</p>

                {form.subjectAssignments.length === 0 && (
                  <p className="text-sm text-gray-400 italic py-2">No subject assignments yet — click "Add Subject".</p>
                )}
                  ...
                  {form.subjectAssignments.map((sa, i) => (
                    <div key={i} className="border rounded-lg p-3 bg-gray-50 relative">
                      ...
                      <div className="mb-3">
                        ...
                        <select className={inputCls} value={sa.subject}
                      ...
                      <div>
                        <label className={labelCls}>Classes taught for this subject</label>
            ...
            <div className="flex gap-1">
              {TABS.map((t) => (
                <button key={t.id} type="button" onClick={() => setTab(t.id)}
                  className={`w-2 h-2 rounded-full transition-colors ${tab === t.id ? 'bg-blue-600' : 'bg-gray-300 hover:bg-gray-400'}`}
