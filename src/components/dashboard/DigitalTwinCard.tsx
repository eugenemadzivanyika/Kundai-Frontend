    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="flex items-start mb-3">
            ...
            <div className="ml-auto text-right">
              <p className="text-[16px] mb-0 text-gray-500 font-bold uppercase tracking-tight">Form {student.form}</p>
              <p className="text-[16px] text-blue-600 font-bold uppercase tracking-tight">Days Remaining: NA</p>
            </div>
          </div>
        ...
        <div className="flex justify-between items-center text-center mt-1 pb-1 w-full">
          {student.attributes?.slice(0, 5).map((attr, index) => (
            <div key={index} className="flex flex-col items-center flex-1 min-w-0 px-0.5">
              {/* Label: Slightly adjusted for better readability at small scale */}
              <p 
                className="text-[10px] text-gray-400 font-bold truncate uppercase tracking-tighter w-full" 
                title={attr.name}
              >
                {attr.name}
              </p>
              
              {/* Value: Increased size slightly to text-xs for the 5-item layout */}
              <p className="text-xs font-black text-gray-800 leading-none mt-0.5">
                {attr.value}%
              </p>
            </div>
          ))}
